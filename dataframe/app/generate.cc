...
void analyze_trip_timestamp(StdDataFrame<uint64_t>& df)
{
    ...
    std::ofstream f1;
    std::cout << "pickup_day size: " << pickup_day_vec.size() << std::endl;
    f1.open("/mnt/data/cfm/dataframe/data/pickup_day.csv");
    f1 << "pickup_day" << std::endl;
    ...
    std::ofstream f2;
    std::cout << "pickup_month size: " << pickup_month_vec.size() << std::endl;
    f2.open("/mnt/data/cfm/dataframe/data/pickup_month.csv");
    f2 << "pickup_month" << std::endl;
    for (auto month : pickup_month_vec) {
        f2 << int(month) << std::endl;
    }
    f2.close();
