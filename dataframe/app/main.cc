...
StdDataFrame<uint64_t> load_data()
{
    return read_csv<-1, int, SimpleTime, SimpleTime, int, double, double, double, int, char, double,
        ...
        "/mnt/data/cfm/dataframe/data/all.csv", "VendorID", "tpep_pickup_datetime", "tpep_dropoff_datetime",
...
void load_trip_timestamp(StdDataFrame<uint64_t>& df) {
    StdDataFrame<uint64_t> day_csv = read_csv<-1, short>("/mnt/data/cfm/dataframe/data/pickup_day.csv", "pickup_day");
    StdDataFrame<uint64_t> month_csv = read_csv<-1, short>("/mnt/data/cfm/dataframe/data/pickup_month.csv", "pickup_month");
...
{ 
    ...
    load_trip_timestamp(df);
    times[7] = std::chrono::steady_clock::now();
    analyze_trip_durations_of_timestamps<short>(df, "pickup_day");
    times[8] = std::chrono::steady_clock::now();
    analyze_trip_durations_of_timestamps<short>(df, "pickup_month");
    ...
    std::cout << "Total: "
              ...
              - std::chrono::duration_cast<std::chrono::microseconds>(times[7] - times[6]).count() 
              << " us" << std::endl;
